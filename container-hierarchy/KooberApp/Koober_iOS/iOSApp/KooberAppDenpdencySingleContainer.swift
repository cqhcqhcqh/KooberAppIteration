//
//  KooberAppDenpdencySingleContainer.swift
//  Koober_iOS
//
...
class KooberAppDenpencySingleContainer {
 ...
 /// let sharedMainViewModel = MainViewModel()
  ...
  /// 为什么是可选的呢？因为当用户登录成功的时候，这个属性就用不上了。
  /// 而且它是 stateful，因此同一个 onboardingViewModel 会在其生命周期内被多个工厂方法共享使用
  /// 为了解决这个烦人的可选问题？后面就要引入 socped container & hierarchy container 来解决
